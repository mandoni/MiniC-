...
public class analizador {
    ...
    public void analizar(){
        ...
        int result;
        String fileName, path, code="", line;
        ...
        if(result == JFileChooser.APPROVE_OPTION){
            try{
                ...
                read.close();
                code = inter.probarLexerFile(code);
                
                String[] ruta = path.split("\\");
                String newPath = fileName.split(".")[0]+".out";
                for(int i = ruta.length-2;i>-1;i--){
                    newPath = ruta[i]+"\\"+newPath;
                }
                BufferedWriter write = new BufferedWriter(new FileWriter(newPath));
